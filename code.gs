...
const SPREADSHEET_ID = '1eFb9MKTtkRnJG_c_eFC_XvCkgJBfccfmVGvbpgDEqc8';
...
function doGet(e) {
  ...
  return template.evaluate()
    .setTitle('Aplikasi Presensi Sekolah')
    ...
    for (let i = 1; i < absensiData.length; i++) {
      ...
      if (rowDateStr === today && rowNisn === scannedNisn) {
        ...
        if (absensiData[i][5]) { 
          ...
          if (minutesDiff < 1) { // Jeda minimal 1 menit
...
function updateAbsensiStatus(token, nisn, nama, kelas, newStatus) {
  try {
    ...
    if (found) {
      absensiSheet.getRange(rowIndex, 7).setValue(newStatus); // Update status
    } else {
      let jamDatang = '-';
      if (newStatus === 'Hadir') {
        jamDatang = Utilities.formatDate(new Date(), 'Asia/Jakarta', 'HH:mm:ss');
      }
      absensiSheet.appendRow([new Date(), "'" + nisn, nama, kelas, jamDatang, '', newStatus]);
    }
...
function getAppConfig() {
  try {
    ...
    let config = {
      ...
      jam_masuk_akhir: '22:30',
      jam_pulang_mulai: '15:00',
      jam_pulang_akhir: '23:00'
    };
